...
お問い合わせフォームからメールを送信するために、Web3Formsを使用しています。

#### 1. Web3Formsの設定

1. **アクセストークンの取得**
   - [Web3Forms](https://web3forms.com/)にアクセス
   - メールアドレスを入力してアクセストークンを取得

2. **アクセストークンの設定**
   - `src/components/sections/Contact.tsx`の`access_key`を更新
   - 現在の設定: `8ee8c8c8-d532-4dcc-980f-06881c075c10`

#### 2. 利点

- **セキュリティ**: 環境変数やSMTP認証情報が不要
- **簡単**: 設定が最小限で済む
- **スパム対策**: 組み込みのスパム対策機能
- **信頼性**: 専用のメール送信インフラ
...
設定後、お問い合わせフォームからテスト送信を行い、`contact@pfp.co.jp` にメールが届くことを確認してください。

#### 4. カスタマイズ

Web3Formsの設定を変更したい場合：
- 送信先メールアドレスの変更
- 件名のカスタマイズ
- スパム対策レベルの調整
- その他のオプション設定
...
- **メール送信機能**: お問い合わせフォームから自動メール送信（contact@pfp.co.jp宛）
